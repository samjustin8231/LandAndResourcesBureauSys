...
namespace Maticsoft.DAL
{
	...
	public partial class T_Batch_Plan
	{
        ...
        /// 增加一条数据
        /// </summary>
        public int Add(Maticsoft.Model.T_Batch_Plan model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into T_Batch_Plan(");
            strSql.Append("batchId,planId,consArea,agriArea,arabArea,issuedQuota,createTime,isSubmited,isDeleted)");
            ...
            parameters[5].Value = model.issuedQuota;
            parameters[6].Value = model.createTime;
            parameters[7].Value = model.isSubmited;
        ...
        /// 更新一条数据
        /// </summary>
        public bool Update(Maticsoft.Model.T_Batch_Plan model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update T_Batch_Plan set ");
		...
		public bool Delete(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from T_Batch_Plan ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public Maticsoft.Model.T_Batch_Plan GetModel(int id)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 * from T_Batch_Plan ");
			strSql.Append(" where id=@id");
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)
			};
			...
			Maticsoft.Model.T_Batch_Plan model=new Maticsoft.Model.T_Batch_Plan();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Maticsoft.Model.T_Batch_Plan DataRowToModel(DataRow row)
        {
            Maticsoft.Model.T_Batch_Plan model = new Maticsoft.Model.T_Batch_Plan();
            if (row != null)
            {
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select T.* ,P.name as planName,B.name as batchName,P.planTypeId as typeId  from T_Batch_Plan T,T_Batch B,T_Plan P WHERE  1=1 and T.batchId=B.id and T.planId = P.id ");
            if (strWhere.Trim() != "")
            {
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
            ...
            strSql.Append(" T.* ,P.name as planName,B.name as batchName,P.planTypeId as typeId  from T_Batch_Plan T,T_Batch B,T_Plan P WHERE  1=1 and T.batchId=B.id and T.planId = P.id ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" and "+strWhere);
			}
		...
		{
			StringBuilder strSql=new StringBuilder();
            strSql.Append("select count(1) FROM T_Batch_Plan T,T_Batch B,T_Plan P WHERE  1=1 and T.batchId=B.id and T.planId = P.id ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" and "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
            ...
            strSql.Append(")AS Row, T.*,B.name as batchName,B.batchTypeId,B.totalArea,B.consArea as consAreaBatch,B.agriArea as agriAreaBatch,B.arabArea as arabAreaBatch,B.hasLevyArea,B.approvalNo,B.approvalTime ,P.name as planName,P.planTypeId as typeId,P.administratorArea from T_Batch_Plan T,T_Batch B,T_Plan P WHERE  1=1 and T.batchId=B.id and T.planId = P.id");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "T_Batch_Plan";
