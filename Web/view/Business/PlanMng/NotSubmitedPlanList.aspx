<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="NotSubmitedPlanList.aspx.cs" Inherits="Maticsoft.Web.View.Business.PlanMng.NotSubmitedPlanList" %>
            ...
            if (id == "" || id==undefined) {
                ...
                $.messager.confirm('提示', '你确定要提交【' + rows.length + '】条记录吗？', function (r) {
                    if (r) {
                        if (rows.length > 0) {
                            ...
                            $.getJSON('PlanHandler.ashx?method=submit', { ids: nos_ids }, function (result) {
                                if (result.flag) {
                                    $('#tt').datagrid('reload'); // reload the user data
                                }
                                $.messager.show({ title: '提示', msg: result.msg });
                ...
                $.messager.confirm('提示', '你确定要提交该记录吗？', function (r) {
                    if (r) {
                        ...
                        $.getJSON('PlanHandler.ashx?method=submit', { ids: id }, function (result) {
                            if (result.flag) {
                                $('#tt').datagrid('reload'); // reload the user data
                            }
                            $.messager.show({ title: '提示', msg: result.msg });
...
<body>
    ...
    <table id="tt" title="计划列表" class="easyui-datagrid" border="false" fit="true" style="width: auto; height: 500px;"        
        idfield="id" pagination="true" data-options="ctrlSelect:true,onDblClickRow:edit,iconCls:'icon-tip',rownumbers:true,url:'PlanHandler.ashx?method=query&isSubmited=0',pageSize:20, pageList:[10,20,30,40,50],method:'get',toolbar:'#tb',striped:true,nowrap:false,onBeforeOpen:beforeDataGridOpen,onLoadSuccess:loadLinkButton" fitcolumns="true"> <%--striped="true"--%>
        <%-- 表格标题--%>
        <thead>
            <tr>
                ...
                <th data-options="field:'consArea',width:120,align:'center',sortable:'true'">建设用地面积</th>   
                ...
                <th data-options="field:'issuedQuota',width:120,align:'center',sortable:'true'">计划指标</th>   
                <th data-options="field:'remainQuota',width:120,align:'center',sortable:'true'">剩余指标</th>   
    ...
    <div id="tb" style="padding: 5px; height: auto">
        ...
        <div style="margin-bottom: 5px">
            ...
            <!-- <input name="" class="scbtn" value="提交" type="button" onclick="submitRecord()">-->
        </div>
    ...
    <div id="dlg" class="easyui-dialog" style="width: 400px; height: auto; padding: 10px 20px"
        ...
        <form id="fm" name="fm" method="post">
            ...
            <div class="fitem">
                <label>计划类型：</label>
                <input name="planTypeId" class="easyui-combobox" style="width:150px;"  data-options="valueField:'id',textField:'name',required:true,url:'../PlanTypeMng/PlanTypeHandler.ashx?method=getList'">
            </div>
            <div class="fitem">
                <label>年份：</label>
            ...
            <div class="fitem">
                <label>下达指标：</label>
                <input name="issuedQuota" class="easyui-numberbox" style="width:150px;" required="required" data-options="">
            </div>
            <div class="fitem">
                <label>剩余指标：</label>
                <input name="remainQuota" class="easyui-numberbox" style="width:150px;" required="required" data-options="">
            </div>
            <div class="fitem">
                <label>文号：</label>
