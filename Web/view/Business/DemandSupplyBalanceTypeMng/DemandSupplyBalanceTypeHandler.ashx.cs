...
namespace Maticsoft.Web.View.Business.DemandSupplyBalanceTypeMng
{
    /// <summary>
    /// DemandSupplyBalanceTypeHandler 的摘要说明
    /// </summary>
    public class DemandSupplyBalanceTypeHandler : IHttpHandler
    {

        BLL.T_DemandSupplyBalanceType bll_demand_supply_balance_type = new BLL.T_DemandSupplyBalanceType();
        ...
        private void IsUsed(HttpContext context)
        {
            ...
            if (BusinessHelper.IsDSBalanceTypeHasUsed(Convert.ToInt32(id)))
            {
                message.flag = true;
            }
            else
        ...
        private void AddOrModify(HttpContext context)
        {
            ...
            if (id == "")
            {
                Model.T_DemandSupplyBalanceType model = new Model.T_DemandSupplyBalanceType();

                model.name = name;
                model.des = des;

                int n = bll_demand_supply_balance_type.Add(model);
                if (n > 0)
                {
                    message.flag = true;
                    message.msg = "添加成功";
            ...
            {
                T_DemandSupplyBalanceType model = bll_demand_supply_balance_type.GetModel(Convert.ToInt32(id));

                model.name = name;
                model.des = des;

                bool flag = bll_demand_supply_balance_type.Update(model);
                if (flag)
                {
                    message.flag = true;
                    message.msg = "修改成功";
        ...
        private void Delete(HttpContext context)
        {
            ...
            bool flag = bll_demand_supply_balance_type.DeleteList(ids);
            if (flag)
            {
                message.flag = true;
                message.msg = "删除成功";
        ...
        private void Query(HttpContext context)
        {
            ...
            //调用分页的GetList方法  
            DataSet ds = bll_demand_supply_balance_type.GetListByPage(strWhere.ToString(), oderby, (page - 1) * pageRows + 1, page * pageRows);


            int count = bll_demand_supply_balance_type.GetRecordCount(strWhere.ToString());//获取条数  
        ...
        private void GetList(HttpContext context)
        {
            ...
            DataSet ds = bll_demand_supply_balance_type.GetList("");
