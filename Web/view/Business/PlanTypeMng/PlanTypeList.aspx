<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="PlanTypeList.aspx.cs" Inherits="Maticsoft.Web.View.Business.PlanTypeMng.PlanTypeList" %>
    ...
    <script type="text/javascript" charset="utf-8">
        ...
        function save() {
            ...
            if (method == "add") {
                ...
                $.getJSON('PlanTypeHandler.ashx', $("#fm").serialize(), function (result) {
                    if (result.flag) {
        ...
        function del() {
            ...
            for (var i = 0; i < rows.length; i++) {
                $.getJSON('PlanTypeHandler.ashx?method=IsUsed', { id: rows[i].id }, function (result) {
                    if (result.flag) {
                        flag = true;
                    }
            ...
            if(flag){
                alert("该类型中已经添加了计划，请先删除计划！"); return;
            }
            ...
            if (rows) {
                $.messager.confirm('提示', '你确定要删除【' + rows.length + '】条记录吗？', function (r) {
                    if (r) {
                        ...
                        $.ajax({
                            url: 'PlanTypeHandler.ashx?method=delete',
                            data: {
                                ids: ids.join(",")
                            },
                            method: 'post',
...
<body>
    <%--表格显示区--%>
    <table id="tt" title="计划类型列表" class="easyui-datagrid" border="false" fit="true" style="width: auto; height: 500px;"        
        idfield="id" pagination="true" data-options="iconCls:'icon-tip',rownumbers:true,url:'PlanTypeHandler.ashx?method=query',pageSize:20, pageList:[10,20,30,40,50],method:'get',toolbar:'#tb',striped:true,nowrap:false" fitcolumns="true"> <%--striped="true"--%>
        <%-- 表格标题--%>
        <thead>
            <tr>
    ...
    <div id="tb" style="padding: 5px; height: auto">
        ...
        <div>
           ...
           名称: 
            <input id="search_name" name="name" class="easyui-textbox" data-options="iconCls:'icon-search',prompt:'计划类型名称'" style="width:160px">&nbsp;&nbsp;
