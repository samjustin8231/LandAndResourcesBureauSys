<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="BatchTypeList.aspx.cs" Inherits="Maticsoft.Web.View.Business.BatchTypeMng.BatchTypeList" %>
    ...
    <script type="text/javascript" charset="utf-8">
        ...
        function save() {
            ...
            if (method == "add") {
                ...
                $.getJSON('BatchTypeHandler.ashx', $("#fm").serialize(), function (result) {
                    if (result.flag) {
        ...
        function del() {
            ...
            for (var i = 0; i < rows.length; i++) {
                $.getJSON('BatchTypeHandler.ashx?method=IsUsed', { id: rows[i].id }, function (result) {
                    if (result.flag) {
                        flag = true;
                    }
            ...
            if (rows) {
                $.messager.confirm('提示', '你确定要删除【' + rows.length + '】条记录吗？', function (r) {
                    if (r) {
                        ...
                        $.ajax({
                            url: 'BatchTypeHandler.ashx?method=delete',
                            data: {
                                ids: ids.join(",")
                            },
                            method: 'post',
        ...
        function getQueryParams(queryParams) {
            ...
            var name = $("#fm_search").find("input[name='searchName']").val();
...
<body>
    <%--表格显示区--%>
    <table id="tt" title="批次类型列表" class="easyui-datagrid" border="false" fit="true" style="width: auto; height: 500px;"        
        idfield="id" pagination="true" data-options="iconCls:'icon-tip',rownumbers:true,url:'BatchTypeHandler.ashx?method=query',pageSize:20, pageList:[10,20,30,40,50],method:'get',toolbar:'#tb',striped:true,nowrap:false" fitcolumns="true"> <%--striped="true"--%>
        <%-- 表格标题--%>
        <thead>
            <tr>
