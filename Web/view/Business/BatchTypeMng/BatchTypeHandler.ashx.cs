...
namespace Maticsoft.Web.View.Business.BatchTypeMng
{
    /// <summary>
    /// BatchTypeHandler 的摘要说明
    /// </summary>
    public class BatchTypeHandler : IHttpHandler
    {
        ...
        private void IsUsed(HttpContext context)
        {
            ...
            if (BusinessHelper.IsBatchTypeHasUsed(Convert.ToInt32(id)))
            {
                message.flag = true;
            }
            else
        ...
        private void AddOrModify(HttpContext context)
        {
            ...
            if (id == "")
            {
                Model.T_BatchType model = new Model.T_BatchType();

                model.name = name;
                model.des = des;

                int n = bll_batch_type.Add(model);
                if (n > 0)
                {
                    message.flag = true;
                    message.msg = "添加成功";
            ...
            {
                T_BatchType model = bll_batch_type.GetModel(Convert.ToInt32(id));

                model.name = name;
                model.des = des;

                bool flag = bll_batch_type.Update(model);
                if (flag)
                {
                    message.flag = true;
                    message.msg = "修改成功";
        ...
        private void Delete(HttpContext context)
        {
            ...
            bool flag = bll_batch_type.DeleteList(ids);
            if (flag)
            {
                message.flag = true;
                message.msg = "删除成功";
        ...
        private void Query(HttpContext context)
        {
            ...
            //调用分页的GetList方法  
            DataSet ds = bll_batch_type.GetListByPage(strWhere.ToString(), oderby, (page - 1) * pageRows + 1, page * pageRows);


            int count = bll_batch_type.GetRecordCount(strWhere.ToString());//获取条数  
        ...
        private void GetList(HttpContext context)
        {
            ...
            DataSet ds = bll_batch_type.GetList("");
