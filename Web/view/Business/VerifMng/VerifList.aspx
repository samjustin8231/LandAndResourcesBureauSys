<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="VerifList.aspx.cs" Inherits="Maticsoft.Web.View.Business.VerifMng.VerifList" %>
    ...
    <script type="text/javascript" charset="utf-8">
        ...
        function onChangeF(newValue, oldValue) {
            ...
            if (year != "" && administratorArea != "") {
                ...
                $.getJSON('VerifHandler.ashx', $("#fm").serialize(), function (result) {
                    if (result.flag) {
                        alert("该行政区的同一年份核销信息已经添加，请重新选择");
                        $("#fm").form("clear"); return;
                    }
                });
            }
        }
            ...
            if (rows) {
                $.messager.confirm('提示', '你确定要删除【' + rows.length + '】条记录吗？', function (r) {
                    if (r) {
                        ...
                        $.ajax({
                            url: 'VerifHandler.ashx?method=delete',
                            data: {
                                ids: ids.join(",")
                            },
                            method: 'post',
...
<body>
    ...
    <table id="tt" title="基本农田核销列表" class="easyui-datagrid" border="false" fit="true" style="width: auto; height: 500px;"        
        idfield="id" pagination="true" data-options="iconCls:'icon-tip',ctrlSelect:true,onDblClickRow:edit,rownumbers:true,url:'VerifHandler.ashx?method=query',pageSize:20, pageList:[10,20,30,40,50],method:'get',toolbar:'#tb',striped:true,nowrap:false,onBeforeOpen:beforeDataGridOpen" fitcolumns="true"> <%--striped="true"--%>
        <%-- 表格标题--%>
        <thead>
            <tr>
    ...
    <div id="dlg" class="easyui-dialog" style="width: 400px; height: auto; padding: 10px 20px"
        ...
        <form id="fm" name="fm" method="post">
            ...
            <div class="fitem">
                ...
                <input name="divisionArea" class="easyui-numberbox" style="width:150px;" required="required" data-options="">
            </div>
            <div class="fitem">
                <label>描述：</label>
