<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="RoleList.aspx.cs" Inherits="Maticsoft.Web.View.RoleMng.RoleList" %>
    ...
    <script type="text/javascript" charset="utf-8">
        ...
        function save() {
            if (method == "add") {
                ...
                $.getJSON('RoleHandler.ashx', $("#fm").serialize(), function (result) {
                    if (result.flag) {
        ...
        function del() {
            ...
            for (var i = 0; i < rows.length; i++) {
                $.getJSON('RoleHandler.ashx?method=IsUsed', { id: rows[i].id }, function (result) {
                    if (result.flag) {
                        flag = true;
                    }
            ...
            if (rows) {
                $.messager.confirm('提示', '你确定要删除【' + rows.length + '】条记录吗？', function (r) {
                    if (r) {
                        ...
                        $.ajax({
                            url: 'RoleHandler.ashx?method=delete',
                            data: {
                                ids: ids.join(",")
                            },
                            method: 'post',
...
<body>
    <%--表格显示区--%>
    <table id="tt" title="角色列表" class="easyui-datagrid" border="false" fit="true" style="width: auto; height: 500px;"        
        idfield="id" pagination="true" data-options="ctrlSelect:true,onDblClickRow:edit,iconCls:'icon-tip',rownumbers:true,url:'RoleHandler.ashx?method=query',pageSize:20, pageList:[10,20,30,40,50],method:'get',toolbar:'#tb',striped:true,nowrap:false" fitcolumns="true"> <%--striped="true"--%>
        <%-- 表格标题--%>
        <thead>
            <tr>
                <th data-options="field:'id',checkbox:true"></th>
                <th data-options="field:'name',width:100,align:'center',sortable:'true'">名称</th>
                <th data-options="field:'contentname',width:120,align:'center',sortable:'true',nowrap:'false'">权限</th>
    ...
    <div id="tb" style="padding: 5px; height: auto">
        ...
        <div>
           ...
           名称: 
            <input id="search_name" name="name" class="easyui-textbox" data-options="iconCls:'icon-search',prompt:'角色名称'" style="width:160px">&nbsp;&nbsp;
    ...
    <div id="dlg" class="easyui-dialog" style="width: 400px; height: auto; padding: 10px 20px"
        data-options="closed:true,buttons:'#dlg-buttons',onOpen:function(){$('#content').combotree('reload','../MenuMng/MenuHandler.ashx?method=GetMenuPrivilege')}"> <%--closed="true" buttons="#dlg-buttons"--%>
        ...
        <form id="fm" method="post">
            ...
            <div class="fitem">
                <label>内容：</label>
                <select id="content" name="content" class="easyui-combotree" style="width:150px;" data-options="url:'../MenuMng/MenuHandler.ashx?method=GetMenuPrivilege',checkbox: true,multiple: true,lines: true"></select>
            </div>
            <div class="fitem">
                <label>描述：</label>
